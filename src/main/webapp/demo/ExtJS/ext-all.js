...
Ext.apply(Ext.EventManager, function() {
	var c, j, e, b, a = Ext.lib.Dom, k = Ext.lib.Event, i = /^(?:scope|delay|buffer|single|stopEvent|preventDefault|stopPropagation|normalized|args|delegate)$/, h = 0, g = 0, d = Ext.isSafari
			? Ext.num(navigator.userAgent.toLowerCase()
					.match(/version\/(\d+\.\d)/)[1]
					|| 2) >= 3.1
...
(function() {
	...
	g.prototype = {
		...
		contains : function(n) {
			return !n ? false : Ext.lib.Dom.isAncestor(this.dom, n.dom
							? n.dom
							: n)
		},
...
Ext.Element.addMethods(function() {
			...
			return {
				...
				wrap : function(e, g) {
					...
					h.dom ? h.dom.appendChild(this.dom) : h
...
Ext.apply(Ext.Element.prototype, function() {
			...
			return {
				insertSibling : function(h, e, g) {
					...
					h = h || {};
					if (h.nodeType || h.dom) {
						d = i.dom.parentNode.insertBefore(c(h), e == "before"
										? i.dom
										: i.dom.nextSibling);
						if (!g) {
							d = a(d)
						...
						if (e == "after" && !i.dom.nextSibling) {
							...
							d = b[e == "after" ? "insertAfter" : "insertBefore"](
									i.dom, h, !g)
						}
					}
					return d
...
Ext.Element.addMethods(function() {
	...
	return {
		...
		removeClass : function(F) {
			...
			if (G.dom.className) {
				Ext.each(F, function(H) {
							G.dom.className = G.dom.className
									.replace(
											b[H] = b[H]
													|| new RegExp(
															"(?:^|\\s+)"
		...
		getStyle : function() {
			return t && t.getComputedStyle ? function(I) {
				...
				I = w(I);
				return (F = H.style[I]) ? F : (G = t.getComputedStyle(H, ""))
						? G[I]
						: null
			} : function(J) {
				var H = this.dom, F, G;
		...
		getHeight : function(H) {
			var G = this, I = G.dom, F = s.max(I.offsetHeight, I.clientHeight)
					|| 0;
			F = !H ? F : F - G.getBorderWidth("tb") - G.getPadding("tb");
			return F < 0 ? 0 : F
		},
		getWidth : function(G) {
			var H = this, I = H.dom, F = s.max(I.offsetWidth, I.clientWidth)
					|| 0;
...
(function		() {
		...
		getFrameWidth : function(c, b) {
			return b && this.isBorderBox() ? 0 : (this.getPadding(c) + this
					.getBorderWidth(c))
		},
...
(function() {
	...
	Ext.Element.addMethods({
				...
				setX : function(m, n) {
					return this.setXY([m, this.getY()], l.call(this, arguments,
									n, 1))
				},
				setY : function(n, m) {
					return this.setXY([this.getX(), n], l.call(this, arguments,
									m, 1))
				},
				setLeft : function(m) {
				...
				request : function(s) {
					...
					if (v.fireEvent(c, v, s)) {
						if (s.el) {
							...
							s.success = (Ext.isFunction(s.success)
									? s.success
									: function() {
						...
						if (m === j
								&& (v.disableCaching && s.disableCaching !== false)
								|| s.disableCaching === true) {
							var u = s.disableCachingParam
						...
						return s.callback ? s.callback
								.apply(s.scope, [s, e, e]) : null
					}
				},
				isLoading : function(m) {
					return m
							? Ext.lib.Ajax.isCallInProgress(m)
							: !!this.transId
				},
				abort : function(m) {
					if (m || this.isLoading()) {
...
Ext.layout.FitLayout = Ext.extend(Ext.layout.ContainerLayout, {
			...
			onLayout : function(a, c) {
				...
				if (!this.container.collapsed) {
					var b = (Ext.isIE6 && Ext.isStrict && c.dom == document.body)
							? c.getViewSize()
							: c.getStyleSize();
		...
		for (g = 0; g < l; g++) {
			...
			if (o.anchor) {
				...
				if (!q) {
					...
					o.anchorSpec = q = {
						right : this
								.parseAnchor(s[0], o.initialConfig.width, d),
						bottom : this.parseAnchor(s[1], o.initialConfig.height,
								p)
					}
				}
				e = q.right ? this.adjustWidthAnchor(q.right(r), o) : undefined;
				b = q.bottom
						? this.adjustHeightAnchor(q.bottom(j), o)
						: undefined;
				if (e || b) {
...
Ext.layout.ColumnLayout = Ext.extend(Ext.layout.ContainerLayout, {
	...
	onLayout : function(d, j) {
		...
		var n = Ext.isIE && j.dom != Ext.getBody().dom ? j.getStyleSize() : j
				.getViewSize();
...
Ext.layout.FormLayout = Ext.extend(Ext.layout.AnchorLayout, {
			...
			getTemplateArgs : function(b) {
				...
				return {
					...
					labelSeparator : a
							? ""
							: (typeof b.labelSeparator == "undefined"
									? this.labelSeparator
									: b.labelSeparator),
...
Ext.layout.VBoxLayout = Ext.extend(Ext.layout.BoxLayout, {
	...
	onLayout : function(e, E) {
		...
		var g = this.getItems(e), j, m, q, r = this.getTargetSize(E), k = r.width
		...
		var c = Math.max(0, y - this.padding.top - this.padding.bottom - C), D = c, z = [], x = [], p = 0, b = Math
				.max(0, k - this.padding.left - this.padding.right);
		Ext.each(g, function(h) {
					if (i && h.flex) {
		...
		Ext.each(g, function(l) {
					...
					if (this.align == "stretch") {
						l.setWidth((B - (j.left + j.right)).constrain(
								l.minWidth || 0, l.maxWidth || 1000000))
					} else {
						if (this.align == "stretchmax") {
							l.setWidth((A - (j.left + j.right)).constrain(
									l.minWidth || 0, l.maxWidth || 1000000))
						} else {
...
Ext.layout.HBoxLayout = Ext.extend(Ext.layout.BoxLayout, {
	...
	onLayout : function(d, E) {
		...
		var e = this.getItems(d), j, c, q, r = this.getTargetSize(E), m = r.width
		...
		var x = Math.max(0, m - this.padding.left - this.padding.right - b), D = x, k = [], A = [], p = 0, a = Math
				.max(0, B - this.padding.top - this.padding.bottom);
		Ext.each(e, function(h) {
					if (i && h.flex) {
		...
		Ext.each(e, function(t) {
					var h = t.margins;
					if (this.align == "stretch") {
						t.setHeight((n - (h.top + h.bottom)).constrain(
								t.minHeight || 0, t.maxHeight || 1000000))
					} else {
						if (this.align == "stretchmax") {
							t.setHeight((s - (h.top + h.bottom)).constrain(
									t.minHeight || 0, t.maxHeight || 1000000))
						} else {
...
Ext.Panel = Ext.extend(Ext.Container, {
	...
	afterRender : function() {
		...
		this.setAutoScroll();
		if (this.html) {
			this.body.update(Ext.isObject(this.html) ? Ext.DomHelper
...
Ext.DatePicker = Ext.extend(Ext.BoxComponent, {
	...
	update : function(H, B) {
		...
		if (this.showToday) {
			...
			var c = (m < u || m > y || (G && A && G.test(m.dateFormat(A))) || (J && J
					.indexOf(m.getDay()) != -1));
			if (!this.disabled) {
...
Ext.extend(Ext.data.Store, Ext.util.Observable, {
			...
			execute : function(d, a, b) {
				...
				if (c !== false) {
					...
					this.proxy.request(Ext.data.Api.actions[d], a, e,
							this.reader, this.createCallback(d, a), this, b)
				}
				return c
			},
			save : function() {
			...
			createCallback : function(b, a) {
				...
				return (b == "read") ? this.loadRecords : function(e, d, g) {
					this["on" + Ext.util.Format.capitalize(b) + "Records"](g,
							a, e);
					if (g === true) {
...
Ext.data.DataReader.prototype = {
	...
	isData : function(a) {
		return (a && Ext.isObject(a) && !Ext.isEmpty(a[this.meta.idProperty]))
				? true
				: false
	}
};
...
Ext.data.DataWriter.prototype = {
	...
	toHash : function(e) {
		var d = e.fields.map, c = {}, b = (this.writeAllFields === false && e.phantom === false)
				? e.getChanges()
				: e.data, a;
			...
			render : function(c, a, d, b) {
				...
				if (this.encode === true) {
					if (Ext.isArray(a) && b[this.meta.idProperty]) {
						d[this.meta.idProperty] = Ext
								.encode(d[this.meta.idProperty])
					}
			...
			readRecords : function(z) {
				...
				for (var u = 0, x = A.length; u < x; u++) {
					...
					for (var t = 0, k = p.length; t < k; t++) {
						var y = p.items[t];
						var g = l.selectValue(Ext
										.value(y.mapping, y.name, true), r,
								y.defaultValue);
						g = y.convert(g, r);
						a[y.name] = g
					}
			...
			createCallback : function(c, a, b) {
				return function(d, e) {
					if (!e.status) {
						...
						this.fireEvent("exception", this, "remote", c, b, e,
								null);
...
Ext.TabPanel = Ext.extend(Ext.Panel, {
	...
	initTab : function(c, a) {
		...
		var b = d ? this.itemTpl.insertBefore(d, e) : this.itemTpl.append(
				this.strip, e);
	...
	getTabEl : function(a) {
		return document.getElementById(this.id + this.idDelimiter
...
Ext.Button = Ext.extend(Ext.BoxComponent, {
	...
	getTemplateArgs : function() {
		var a = (this.cls || "");
		a += (this.iconCls || this.icon) ? (this.text
				? " x-btn-text-icon"
...
(function() {
	...
	Ext.extend(a, Ext.Container, {
				...
				constructButton : function(d) {
					var c = d.events ? d : this.constructItem(d, d.split
									? "splitbutton"
									: this.defaultType);
...
Ext.ToolTip = Ext.extend(Ext.Tip, {
	...
	getTargetXY : function() {
		if (this.anchor) {
			...
			var j = (this.anchorToTarget && !this.trackMouse)
	...
	constrainScroll : function(c) {
		...
		if (this.floating) {
			a = this.maxHeight ? this.maxHeight : Ext
					.fly(this.el.dom.parentNode).getViewSize().height
					- c
		} else {
			a = this.getHeight()
		}
...
Ext.form.Field = Ext.extend(Ext.BoxComponent, {
			...
			adjustWidth : function(a, b) {
				if (typeof b == "number"
						&& (Ext.isIE && (Ext.isIE6 || !Ext.isStrict))
...
Ext.FormPanel = Ext.extend(Ext.Panel, {
			...
			onRemove : function(a, b) {
				if (this.isField(b)) {
					...
					if (b.findByType) {
						Ext.each(b.findBy(this.isField), this.form.remove,
								this.form)
					}
				}
			},
	...
	initComponent : function() {
		...
		if (!this.store) {
			...
			var a = this.parseDate(this.maxValue)
					|| new Date(this.initDate).clearTime().add("mi",
							(24 * 60) - 1);
			var c = [];
			while (b <= a) {
	...
	handleHdMove : function(h, d) {
		if (this.activeHd && !this.headersDisabled) {
			...
			if (a - g.left <= b && this.cm.isResizable(this.activeHdIndex - 1)) {
				c.cursor = Ext.isAir ? "move" : Ext.isWebKit
						? "e-resize"
						: "col-resize"
			} else {
...
Ext.extend(Ext.grid.RowSelectionModel, Ext.grid.AbstractSelectionModel, {
			...
			onEditorKey : function(i, h) {
				...
				if (c == h.TAB) {
					...
					b.completeEdit();
					if (a) {
						j = d.walkCells(b.row, b.col - 1, -1, this.acceptsNav,
								this)
					} else {
						j = d.walkCells(b.row, b.col + 1, 1, this.acceptsNav,
								this)
...
Ext.grid.TemplateColumn = Ext.extend(Ext.grid.Column, {
			constructor : function(a) {
				...
				var b = typeof Ext.isObject(this.tpl)
						? this.tpl
	...
	doRender : function(d, j, t, a, s, v) {
		...
		for (x = 0, y = j.length; x < y; x++) {
			...
			if (!n || n.group != z) {
				...
				var c = typeof this.state[p] !== "undefined"
						? !this.state[p]
						: this.startCollapsed;
		...
		for (x = 0, y = B.length; x < y; x++) {
			...
			A[A.length] = Ext.grid.GroupingView.superclass.doRender.call(this,
					d, z.rs, t, z.startRow, s, v);
